...
| e_deck_handshake | 握手包   | 0x02   | 4Bytes   | 用于潜器主控与甲板软件之间建立通讯握手<br>甲板自检时潜器主控主动发出握手包，数据字段为uint32_t类型，表示发送的序号，每包加1，频率1Hz<br>甲板软件收到握手包后回复相同内容的握手包<br>（通讯电台） |
...
| 枚举定义                | 功能名称     | 功能ID | 数据长度 | 说明                                                         |
| ----------------------- | ------------ | ------ | -------- | ------------------------------------------------------------ |
| e_propeller_power_off   | 下电         | 0x00   | 0        | 主推进器下电                                                 |
| e_propeller_power_on    | 上电         | 0x01   | 0/1Byte  | **甲板软件->主控**：<br/>主推进器上电自检<br/>**主控->甲板软件**：<br/>主推进器自检反馈信息<br/>0-自检成功，非0-自检失败 |
...
| e_can_motor_power_on    | 上电         | 0x01   | 1Byte/2Bytes | **甲板软件->主控**：<br>关节电机上电自检命令<br>数据：关节组合ID（）<br>**主控->甲板软件**：<br>反馈关节电机自检状态<br>数据2Bytes，uint8_t，第一个字节指示关节组合ID，第二个字节指示自检状态，0-自检成功，非0-自检失败 |
